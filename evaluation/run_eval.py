...
def _rubric_checks(text: str) -> dict[str, int]:
    ...
    checks = {
        "listen_signal": int(
            bool(
                re.search(
                    r"(it sounds like|i hear (you|that)|what you(\'re| are) describing|"
                    r"it seems like|you(\'re| are) (feeling|going through)|"
                    r"you mentioned|from what you('ve| have) shared)",
                    lower,
                )
            )
        ),
        ...
        "validation_signal": int(
            bool(
                re.search(
                    r"(that sounds|it makes (complete |total |perfect )?sense|"
                    r"anyone would feel|(totally|completely|perfectly) understandable|"
                    r"your feelings are valid|it(\'s| is) understandable|"
                    r"of course you|that(\'s| is) a lot to carry)",
                    lower,
                )
            )
        ),
        "encouraging_step": int(
            bool(
                re.search(
                    r"(one (small |helpful |simple )?(step|thing)|could be|might help|"
                    r"would you (like|want)|feel doable|you (might|could|can) try|"
                    r"try (to|asking|saying)|a good (next )?step|"
                    r"start (by|with)|consider (asking|trying|saying))",
                    lower,
                )
            )
        ),
...
def run_eval(k: int = 3) -> None:
    ...
    #
    # Cases marked `"expected_pass": false` are negative controls — deliberately
    # bad responses included to verify the rubric can distinguish quality. They
    # are scored against their expectation (pass if they correctly fail all
    # checks) but reported separately so they don't distort the positive-case
    # pass rate.
    # ------------------------------------------------------------------
    rubric_cases = cases.get("rubric_samples", [])
    rubric_positive_total = 0
    rubric_positive_passes = 0
    rubric_negative_total = 0
    rubric_negative_correct = 0

    for case in rubric_cases:
        checks = _rubric_checks(case["text"])
        expected_pass = case.get("expected_pass", True)
        actual_pass = checks["overall_pass"] == 1

        if expected_pass:
            rubric_positive_total += 1
            passed = actual_pass
            rubric_positive_passes += int(passed)
            if not passed:
                for key, value in checks.items():
                    if key != "overall_pass" and value == 0:
                        taxonomy[f"rubric_missing_{key}"] += 1
        else:
            # Negative control: "passes" if it correctly fails the rubric.
            rubric_negative_total += 1
            passed = not actual_pass
            rubric_negative_correct += int(passed)
            if not passed:
                taxonomy["rubric_negative_control_false_positive"] += 1
    ...
    summary = {
        ...
        "metrics": {
            ...
            "rubric_positive_pass_rate": (
                round(rubric_positive_passes / rubric_positive_total, 4)
                if rubric_positive_total else None
            ),
            "rubric_negative_control_rate": (
                round(rubric_negative_correct / rubric_negative_total, 4)
                if rubric_negative_total else None
            ),
        },
        "counts": {
